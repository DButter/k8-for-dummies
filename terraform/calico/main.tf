...
module "initial_master" {
  ...
  template_configs = [
    {
      filename = "./calico/init.sh.tpl"
      template_vars = {
        public_key = replace(data.local_file.public_key.content, "\n", "")
      }
    },
    {
      filename = "./calico/kubeadm.sh.tpl"
      template_vars = {
    ...
    {
      filename = "./calico/updateKernel.sh.tpl"
    }
  ]
  rendered_configs = [
    {
...
module "masters" {
  ...
  template_configs = [
    ...
    {
      filename = "./calico/joinMaster.sh.tpl"
      template_vars = {
        secret_manager_id = module.base.aws_secretsmanager_secret_arn,
        node_name         = "${each.key}.k8.local"
      }
...
module "nodes" {
  ...
  template_configs = [
    ...
    {
      filename = "./calico/joinWorker.sh.tpl"
      template_vars = {
        secret_manager_id = module.base.aws_secretsmanager_secret_arn,
        node_name         = "${each.key}.k8.local"
      }
