...
module "initial_master" {
  ...
  template_configs = [
    {
      filename = "./flannel/init.sh.tpl"
      template_vars = {
        public_key = replace(data.local_file.public_key.content, "\n", "")
      }
    },
    {
      filename = "./flannel/kubeadm.sh.tpl"
      template_vars = {
    ...
    {
      filename = "./flannel/updateKernel.sh.tpl"
    }
  ]
  rendered_configs = [
    {
      filename = "./flannel/installFlannelWireguard.sh.tpl"
      template_vars = {
        pod_network_cidr = local.pod_network_cidr
      }
    }
...
module "masters" {
  ...
  template_configs = [
    ...
    {
      filename = "./flannel/joinMaster.sh.tpl"
      template_vars = {
        secret_manager_id = module.base.aws_secretsmanager_secret_arn,
        node_name         = "${each.key}.k8.local"
      }
...
module "nodes" {
  ...
  template_configs = [
    ...
    {
      filename = "./flannel/joinWorker.sh.tpl"
      template_vars = {
        secret_manager_id = module.base.aws_secretsmanager_secret_arn,
        node_name         = "${each.key}.k8.local"
      }
