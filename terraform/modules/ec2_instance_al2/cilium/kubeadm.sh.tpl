...
sudo kubeadm init --control-plane-endpoint=plane.k8.local --node-name=${node_name} --token=$TOKEN --cri-socket=unix:///run/containerd/containerd.sock --pod-network-cidr=${pod_network_cidr} --upload-certs --certificate-key=$certificateKey --skip-phases=addon/kube-proxy
